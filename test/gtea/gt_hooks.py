...
class GreenteaCliTestHookTest(unittest.TestCase):
    ...
    def test_format_before_run(self):
        ...
        command = LcovHook.format_before_run(
            "command {expansion}", {"expansion": expansions}, verbose=True
        )
    ...
    def test_expand_parameters_with_1_list(self):
        ...
        self.assertEqual(
            ...
            self.cli_hooks.expand_parameters(
                "[{token_list}]", {"token_list": ["new_value_1", "new_value_2"]}
            ),
        )

        # List with prefix
        self.assertEqual(
            "-a new_value_1 -a new_value_2",
            self.cli_hooks.expand_parameters(
                "[-a {token_list}]", {"token_list": ["new_value_1", "new_value_2"]}
            ),
        )

        # List with prefix and extra text
        self.assertEqual(
            "-a /path/to/new_value_1 -a /path/to/new_value_2",
            self.cli_hooks.expand_parameters(
                "[-a /path/to/{token_list}]",
    ...
    def test_expand_parameters_with_2_lists(self):
        ...
        self.assertEqual(
            "ytA T1 ytA T2 ytA T3 ytB T1 ytB T2 ytB T3 ytC T1 ytC T2 ytC T3",
            self.cli_hooks.expand_parameters(
                "[{yt_target_list} {test_list}]",
                {
                    "test_list": ["T1", "T2", "T3"],
        ...
        self.assertEqual(
            "lcov -a /build/{yotta_target_name}/mbed-drivers-test-basic.info -a /build/{yotta_target_name}/mbed-drivers-test-hello.info",
            self.cli_hooks.expand_parameters(
                "lcov [-a /build/{yotta_target_name}/{test_list}.info]",
                {
    ...
    def test_expand_parameters_exceptions(self):
        ...
        self.assertEqual(
            None,
            self.cli_hooks.expand_parameters(
                "some text without square brackets but with tokes: {test_list} and {yotta_target_name}",
                {
                    "test_list": ["mbed-drivers-test-basic", "mbed-drivers-test-hello"],
                    "yotta_target_name": "frdm-k64f-gcc",
                },
    ...
    def test_format_before_run_with_1_list_1_string(self):
        ...
        self.assertEqual(
            "build path = -a /build/frdm-k64f-gcc/mbed-drivers-test-basic -a /build/frdm-k64f-gcc/mbed-drivers-test-hello",
            self.cli_hooks.format_before_run(
                "build path = [-a /build/{yotta_target_name}/{test_name_list}]",
                {
                    "test_name_list": [
    ...
    def test_check_if_file_exists(self, pathGetsize_mock, pathExists_mock):
        ...
        self.assertEqual(
            "-a ./build/frdm-k64f-gcc.info",
            self.lcov_hooks.check_if_file_exists_or_is_empty(
                "(-a <<./build/frdm-k64f-gcc.info>>)"
            ),
        )
