...
"""
    ...
    def test_parse_ctesttestfile_line(self):
        ...
        for line in self.ctesttestfile.splitlines():
            line_parse = cmake_handlers.parse_ctesttestfile_line(
                LINK_TARGET, BINARY_TYPE, line, verbose=True
            )
            if line_parse:
    ...
    def test_list_binaries_for_targets(self):
        ...
        def run_and_capture(path, verbose=False):
            ...
            cmake_handlers.list_binaries_for_targets(
                build_dir=path, verbose_footer=verbose
            )
