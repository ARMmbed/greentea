...
def build_with_yotta(
    yotta_target_name, verbose=False, build_to_release=False, build_to_debug=False
):
    ...
    if yotta_result:
        gt_logger.gt_log(
            "yotta build for target '%s' was successful"
...
def get_platform_name_from_yotta_target(target):
    ...
    with open(target_json_path, "r") as f:
        ...
        try:
            ...
            gt_logger.gt_log_err(
                "Failed to load json data from target.json! error [%s]\n" % str(e)
    ...
    if "keywords" not in target_json:
        gt_logger.gt_log_err(
            "No 'keywords' in target.json! Can not determine required mbed platform name!"
        )
        return None
    ...
    if platform_name is None:
        gt_logger.gt_log_err(
            'No keyword with format "mbed-target:<platform name>" found in target.json!\n'
            + "Can not determine required mbed platform name!"
        )
        return None
...
def get_test_spec_from_yt_module(opts):
    ...
    if opts.list_of_targets:
        ...
        if current_target:
            gt_logger.gt_log(
                "assuming default target as '%s'" % gt_logger.gt_bright(current_target)
            )
        ...
        for mapping in opts.map_platform_to_yt_target.split(","):
            if len(mapping.split(":")) == 2:
                ...
                gt_logger.gt_log_tab(
                    ...
                    % (gt_logger.gt_bright(yt_target), gt_logger.gt_bright(platform))
                )
            else:
                gt_logger.gt_log_tab(
                    "unknown format '%s', use 'target:platform' format" % mapping
                )
