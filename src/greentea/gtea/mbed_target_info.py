...
def get_mbed_targets_from_yotta_local_module(
    mbed_classic_name, yotta_targets_path="./yotta_targets"
):
    ...
    target_dirs = [
        target_dir_name
        for target_dir_name in os.listdir(yotta_targets_path)
        if os.path.isdir(os.path.join(yotta_targets_path, target_dir_name))
    ]

    gt_logger.gt_log(
        "local yotta target search in '%s' for compatible mbed-target '%s'"
        % (
            gt_logger.gt_bright(yotta_targets_path),
    ...
    for target_dir in target_dirs:
        ...
        try:
            with open(path, "r") as data_file:
                ...
                yotta_target_name = parse_mbed_target_from_target_json(
                    mbed_classic_name, target_json_data
                )
                if yotta_target_name:
                    target_dir_name = os.path.join(yotta_targets_path, target_dir)
                    gt_logger.gt_log_tab(
                        "inside '%s' found compatible target '%s'"
                        % (
                            gt_logger.gt_bright(target_dir_name),
...
def get_mbed_targets_from_yotta(mbed_classic_name):
    ...
    cmd = [
        ...
        "search",
        "-k",
        "mbed-target:%s" % mbed_classic_name.lower().strip(),
        "target",
    ]
    gt_logger.gt_log(
        "yotta search for mbed-target '%s'"
        % gt_logger.gt_bright(mbed_classic_name.lower().strip())
    )
    gt_logger.gt_log_tab("calling yotta: %s" % " ".join(cmd))
    _stdout, _stderr, _ret = run_cli_process(cmd)
        ...
        for line in _stdout.splitlines():
            ...
            if yotta_target_name:
                if yotta_target_name and yotta_target_name not in result:
                    result.append(yotta_target_name)
                    gt_logger.gt_log_tab(
                        "found target '%s'" % gt_logger.gt_bright(yotta_target_name)
                    )
    else:
...
def add_target_info_mapping(
    mbed_classic_name, map_platform_to_yt_target=None, use_yotta_registry=False
):
    ...
    if map_platform_to_yt_target and mbed_classic_name in map_platform_to_yt_target:
        yotta_target_search = list(
            set(yotta_target_search + map_platform_to_yt_target[mbed_classic_name])
        )
    ...
    for new_yt_target in yotta_target_search:
        ...
        TARGET_INFO_MAPPING[mbed_classic_name]["yotta_targets"].append(
            {"yotta_target": new_yt_target, "mbed_toolchain": mbed_toolchain}
        )

    return TARGET_INFO_MAPPING


def get_mbed_clasic_target_info(
    mbed_classic_name, map_platform_to_yt_target=None, use_yotta_registry=False
):
    ...
    """
    TARGET_INFO_MAPPING = add_target_info_mapping(
        mbed_classic_name, map_platform_to_yt_target, use_yotta_registry
    )
    return (
        TARGET_INFO_MAPPING[mbed_classic_name]
        if mbed_classic_name in TARGET_INFO_MAPPING
        else None
    )
